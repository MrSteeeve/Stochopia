...
from .types import EpisodeTask, StepTransition


TRAJECTORY_FORMAT = "mirage-trajectory-json-v1"
...
class TrajectoryRecorder:
    ...
    @property
    def transitions(self) -> tuple[StepTransition, ...]:
    ...
    def record(
        ...
        *,
        ...
        if self._initial_state_hash is None:
            self._initial_state_hash = transition.state_hash_before
        if not self._entries:
            if transition.state_hash_before != self._initial_state_hash:
            ...
            if transition.state_hash_before != previous.transition.state_hash_after:
                raise ValueError("transition state hashes are not continuous")
            previous_record_hash = previous.record_hash

        info_hash = transition.info.get("state_hash")
        if info_hash is not None and str(info_hash) != transition.state_hash_after:

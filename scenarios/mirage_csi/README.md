...
```bash
python -m mirage.benchmark_cli validate-market path/to/market_snapshots.csv
```

Run the deterministic example quote:

```bash
python -m mirage.benchmark_cli demo market_snapshots.example.csv \
...
is `mirage.role_config.load_role_specs`'s default `base_dir`, so the
`system_prompt_file` paths in `benchmark_roles.yaml` are written relative to
the repository root (e.g. `scenarios/mirage_csi/prompts/client_main.md`), not
...
frozen model/temperature combination (`main_npc_lineup_id: npc-fixed-v1` in

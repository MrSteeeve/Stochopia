...
ROLE_PROTOCOL_VERSION = "stochopia.csi.legacy.v1"
...
def _require_protocol_version(data: dict, cfg_path: Path) -> str:
    protocol_version = str(_require(data, "protocol_version", str(cfg_path)))
    if protocol_version != ROLE_PROTOCOL_VERSION:
        raise RoleConfigError(
            f"角色配置 protocol_version 必须是 {ROLE_PROTOCOL_VERSION!r}"
            f"（{cfg_path}），实际为 {protocol_version!r}"
        )
...
def load_judges_config(path: str | Path, registry: ModelRegistry) -> JudgesConfig:
    ...
    _require_protocol_version(data, cfg_path)
...
def load_role_specs(
    ...
    *,
    ...
    ``scenarios/stochopia_csi/prompts/…`` 这类仓库相对路径可直接命中。
    ...
    protocol_version = _require_protocol_version(data, cfg_path)
    ...
    for role_id, raw in roles_node.items():
        spec = _parse_role(
            str(role_id), raw, registry, resolved_base, protocol_version
        )

...
from .role_config import ROLE_PROTOCOL_VERSION, RoleSpecV2
from .stats import derive_seed

SEED_NAMESPACE = "stochopia.seed.env-agents.v1"
CACHE_INFERENCE_CONTRACT = "stochopia.env-response.v1"
...
class RoleResponse:
    ...
    timeout|provider|format|grounding|protocol。``degraded`` 为 True 表示这是容错兜底，
...
def parse_client_response(raw: str) -> _Parsed:
    """解析 stochopia.role.client-response.v1。"""
    obj = _load_obj(raw)
...
def parse_risk_response(raw: str) -> _Parsed:
    """解析 stochopia.role.risk-response.v1。"""
    obj = _load_obj(raw)
    action = obj.get("action")
...
def parse_desk_response(raw: str) -> _Parsed:
    """解析 stochopia.role.trading-desk-response.v1。"""
    obj = _load_obj(raw)
    action = obj.get("action")
...
_PARSERS = {
    "stochopia.role.client-response.v1": parse_client_response,
    "stochopia.role.risk-response.v1": parse_risk_response,
    "stochopia.role.trading-desk-response.v1": parse_desk_response,
}
...
class EnvResponseCache:
    """角色响应缓存：key=sha256(contract|protocol|role|inference|messages)。
    ...
    def make_key(
        ...
        *,
        ...
        output_schema: str = "",
        inference_contract: str = CACHE_INFERENCE_CONTRACT,
    ) -> str:
        canonical = "\x1f".join(
            [
                inference_contract,
...
class FrozenEnvAgent:
    ...
    def _render_request(self, request: RoleRequest) -> str:
        ...
        return (
            f"【请求类型】{request.kind}\n"
            f"【协议版本】{request.protocol_version}\n"
    ...
    async def respond(
        ...
        """
        if request.protocol_version != self.spec.protocol_version:
            return self._degraded(request, "protocol")
